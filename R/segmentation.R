...
#' @param min_segs must be an integer specifying the minimum number of segments to extract, defaults to 3
#' @param max_segs must be an integer specifying the maximum number of segments to extract, defaults to 5
#' @param weight must be a character vector of the name of the row weight or NULL
#' @param method must be a character vector of at least one of 'factor' or 'lca'
#' @examples
#' mydf <- data.frame(col1=c(1,2,3),col2=c(1,3,2),col3=c(1,2,1))
#' segmentation(df = mydf, vars = c('col1','col2','col3'),min_segs = 3, max_segs = 5,method = c('factor'))
#' @export

segmentation <- function(df, vars, min_segs = 3, max_segs = 5, weight_var = NULL, method = c('factor')){
